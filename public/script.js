function toggleFilter(id) {
    ...
    if (btn) btn.classList.toggle('filter-open');
}
...
function updateCartTotals() {
    ...
    items.forEach(item => {
        ...
        const qty = qtyEl ? parseInt(qtyEl.value || qtyEl.textContent) : 0;        subtotal += price * qty;
    });
...
async function addToCart(btn, productId) {
    ...
    if (!csrfTokenMeta) return console.error('CSRF token not found');
...
async function logoutUser() {
    ...
    if (token) {
        ...
        } catch (e) { console.log('Server failed'); }
    }
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            if (response.status === 201) {
                ...
                alert('Error: ' + (result.message ?? 'Check fields'));
            }
        });
    }
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function () {
            ...
            searchTimeout = setTimeout(() => {
                ...
                const isAdmin = window.location.pathname.includes('/admin/products');
                ...
                fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                    ...
                    .then(html => {
                        ...
                        const targetId = isCatalog ? 'catalogResults' : 'adminProductsTable';
                        const currentC = document.getElementById(targetId);
                        const newC = doc.getElementById(targetId);

                        if (currentC && newC) currentC.innerHTML = newC.innerHTML;
